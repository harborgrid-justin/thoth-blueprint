...
import { createId, type PlanElement, type Point, type Polygon, type RightOfWay, type LandUseCategory } from "@thoth/domain";
...
function parseGeoJSONPolygon(coords: unknown): Point[] {
  ...
  const limit = ring.length > 1 && first && last &&
    first[0] === last[0] && 
    first[1] === last[1] 
    ? ring.length - 1 
    : ring.length;
...
export function geojsonToElements(
  ...
  for (const feature of geojson.features) {
    ...
    const layerId = (properties.layerId as string) || getDefaultLayerForKind(kind);
    ...
    try {
      if (feature.geometry.type === "Polygon") {
        ...
        const position = reprojectPoint({ x: coords[0], y: coords[1] }, sourceCrs, projectCrs);

        elements.push(buildPointElement(kind, name, layerId, position, properties));
      }
    } catch (err) {
...
export function elementsToGeojson(
  ...
  for (const el of elements) {
    ...
    } else if ("position" in el && el.position && typeof el.position === "object") {
...
function buildPolygonElement(
  ...
  switch (kind) {
    case "parcel":
      return { id, kind: "parcel", name, layerId, boundary, apn: (props.apn as string) || "" };
    case "zone":
      return {
        id,
        kind: "zone",
        ...
        allowedUses: (props.allowedUses as LandUseCategory[]) || ["residential"],
    ...
    case "landuse":
      return { id, kind: "landuse", name, layerId, boundary, category: (props.category as LandUseCategory) || "residential" };
    case "lot":
      return { id, kind: "lot", name, layerId, boundary, setback: (props.setback as number) || 3 };
    case "building":
      return {
        id,
        kind: "building",
