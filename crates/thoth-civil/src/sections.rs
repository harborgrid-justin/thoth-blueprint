...
use crate::alignment::{resolve_alignment, point_at_station, HorizontalAlignment, ResolvedAlignment};
...
/// range.
pub fn generate_sample_lines(alignment: &HorizontalAlignment, resolved: Option<&ResolvedAlignment>, interval: f64, swath_width: f64) -> SampleLineGroup {
    let owned;
    ...
    let Some(res) = res else {
        return SampleLineGroup { id: format!("slg-{}", alignment.id), name: format!("{} Sample Lines", alignment.name), alignment_id: alignment.id.clone(), sample_lines: Vec::new() };
    };
    ...
    while s <= end + 1e-6 {
        if let Ok(at) = point_at_station(res, s) {
            sample_lines.push(SampleLine { station: s, swath_left: swath_width, swath_right: swath_width, center_point: at.point });
        }
        s += interval;
    }

    SampleLineGroup { id: format!("slg-{}", alignment.id), name: format!("{} Sample Lines", alignment.name), alignment_id: alignment.id.clone(), sample_lines }
}
...
/// `sampleCrossSection`).
pub fn generate_section_view(sample_line: &SampleLine, existing_grid: &ElevationGrid, proposed_grid: Option<&ElevationGrid>, resolved: &ResolvedAlignment) -> Option<SectionView> {
    let cs = sample_cross_section(existing_grid, proposed_grid, resolved, sample_line.station, sample_line.swath_left, 2.5)?;

    let all_elevs: Vec<f64> = cs.existing_points.iter().map(|p| p.elevation).chain(cs.proposed_points.iter().map(|p| p.elevation)).collect();
    ...
    let (min_elevation, max_elevation) = if all_elevs.is_empty() { (0.0, 100.0) } else { (min_elevation, max_elevation) };

    Some(SectionView { station: sample_line.station, sample_line_id: format!("sl-{}", sample_line.station), cross_section: cs, min_elevation, max_elevation })
}

fn calc_section_areas(existing_grid: &ElevationGrid, proposed_grid: &ElevationGrid, resolved: &ResolvedAlignment, line: &SampleLine) -> (f64, f64) {
    let Some(cs) = sample_cross_section(existing_grid, Some(proposed_grid), resolved, line.station, line.swath_left, 2.0) else {
        return (0.0, 0.0);
    };
...
pub fn calculate_earthwork_volumes(sample_line_group: &SampleLineGroup, existing_grid: &ElevationGrid, proposed_grid: &ElevationGrid, resolved: &ResolvedAlignment) -> QtoVolumeSummary {
    let mut items = Vec::new();
    let mut cum_cut = 0.0;
    let mut cum_fill = 0.0;

    let lines = &sample_line_group.sample_lines;
    let section_areas: Vec<(f64, f64, f64)> = lines.iter().map(|line| { let (cut, fill) = calc_section_areas(existing_grid, proposed_grid, resolved, line); (line.station, cut, fill) }).collect();

    for i in 0..section_areas.len() {
        let (station, cut_sq_ft, fill_sq_ft) = section_areas[i];

        if i == 0 {
            items.push(EarthworkVolumeItem { station, cut_area_sq_ft: cut_sq_ft, fill_area_sq_ft: fill_sq_ft, cut_volume_cu_yd: 0.0, fill_volume_cu_yd: 0.0, net_volume_cu_yd: 0.0, cumulative_cut_cu_yd: 0.0, cumulative_fill_cu_yd: 0.0, cumulative_net_cu_yd: 0.0 });
            continue;
        }
    ...
    QtoVolumeSummary { alignment_id: sample_line_group.alignment_id.clone(), items, total_cut_cu_yd: cum_cut, total_fill_cu_yd: cum_fill, total_net_cu_yd: cum_fill - cum_cut }
}
...
mod tests {
    ...
    fn generate_sample_lines_spans_the_alignment_at_interval() {
        let align = HorizontalAlignment::new("a1", "Test", vec![AlignmentPi::simple(Point::new(0.0, 0.0)), AlignmentPi::simple(Point::new(1000.0, 0.0))], 0.0);
    ...
    fn generate_sample_lines_on_degenerate_alignment_is_empty() {
        let align = HorizontalAlignment::new("a1", "Test", vec![AlignmentPi::simple(Point::ZERO)], 0.0);
        let group = generate_sample_lines(&align, None, 100.0, DEFAULT_SWATH_WIDTH);
        assert!(group.sample_lines.is_empty());
    }
    ...
    fn calculate_earthwork_volumes_matches_average_end_area_by_hand() {
        let align = HorizontalAlignment::new("a1", "Test", vec![AlignmentPi::simple(Point::new(0.0, 0.0)), AlignmentPi::simple(Point::new(200.0, 0.0))], 0.0);
        let resolved = resolve_alignment(&align).unwrap();

        // Flat existing at 10, flat proposed at 12: uniform 2-unit fill everywhere.
        let existing = ElevationGrid::new(Point::new(-60.0, -60.0), 5.0, 30, 30, vec![10.0; 900]).unwrap();
        let proposed = ElevationGrid::new(Point::new(-60.0, -60.0), 5.0, 30, 30, vec![12.0; 900]).unwrap();
