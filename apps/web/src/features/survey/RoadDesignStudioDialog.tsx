...
export const RoadDesignStudioDialog: React.FC<RoadDesignStudioDialogProps> = ({
  ...
  return (
    <DialogShell
      ...
      title="Road Design & Traceability Studio"
      description="REQ-10 through REQ-21 AASHTO Geometric Highway Design Solver"
      maxWidthClass="max-w-5xl"
      footer={
        <div className="flex w-full items-center justify-between text-xs text-muted-foreground">
          <span>Domain Solvers: ISO/IEC/IEEE 29148 Compliant</span>
          <Button onClick={onClose} className={SURVEY_STYLES.btnPrimary}>
            Close Studio
          </Button>
        </div>
      }
        ...
        <div className={SURVEY_STYLES.tabBar}>
          {[
            ...
            <button
              ...
              className={cn(
                SURVEY_STYLES.btnTab,
                activeTab === tab.id && SURVEY_STYLES.btnTabActive
              )}
            >
              {tab.label}
            </button>
              ...
              <div className="mt-3 space-y-2">
                {alignmentChecks.map((c, i) => (
                  <div
                    key={i}
                    className={cn(
                      "flex items-center justify-between rounded p-3 font-mono text-xs",
                      c.isViolation
        ...
        {activeTab === "assembly" && (
          <div className="space-y-4">
            <div className={SURVEY_STYLES.card}>
              ...
              <pre className="mt-3 overflow-x-auto rounded border border-border bg-background p-3 font-mono text-xs text-amber-300">
                ...
                <ul className="mt-3 space-y-1 font-mono text-xs text-muted-foreground">
                  {featureLines.map((fl, i) => (
                    <li key={i} className="flex justify-between border-b border-border/50 py-1">
                      ...
                      <span className="font-bold text-amber-400">{fl.points.length} points</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {activeTab === "intersection" && (
          <div className="space-y-4">
            <div className={SURVEY_STYLES.card}>
              ...
              <div className="rounded border border-border bg-background p-3 font-mono text-xs text-amber-400">
                Fastest Path Entry Speed: {((roundaboutAnalysis as any).v1 ?? (roundaboutAnalysis as any).entrySpeed ?? 22.5).toFixed(1)} mph
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "sections" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className={SURVEY_STYLES.label}>Earthwork End-Area QTO Quantities</span>
              <span className="text-xs text-muted-foreground">
                Total Cut: <b className="text-rose-400">{(earthworkQTO?.totalCutCuYd ?? 0).toFixed(1)} yd³</b> | Total Fill: <b className="text-emerald-400">{(earthworkQTO?.totalFillCuYd ?? 0).toFixed(1)} yd³</b>
              </span>
            </div>
            <div className={SURVEY_STYLES.card}>
