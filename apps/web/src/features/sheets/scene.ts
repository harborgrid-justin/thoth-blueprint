...
import type { HatchPattern } from "@thoth/domain";
...
export type SheetPrimitive =
  ...
  | { t: "polyline"; pts: Pt[]; w?: number; color?: string; dash?: number[]; close?: boolean }
  | {
      t: "polygon";
      pts: Pt[];
      w?: number;
  ...
  | { t: "circle"; c: Pt; r: number; sw?: number; stroke?: string; fill?: string; fillOpacity?: number }
  | {
      t: "text";
      at: Pt;
      text: string;
...
export function paperToPointsForSheet(value: number, unit: "in" | "mm"): number {
...
export function hatchLines(poly: Pt[], pattern: HatchPattern): SheetPrimitive[] {
  ...
  const dirs: number[] = pattern.kind === "crosshatch" || pattern.kind === "grid"
...
export function arrowHead(at: Pt, dir: Pt, size: number, color = INK): SheetPrimitive {
  const nx = -dir.y;
  const ny = dir.x;
...
export function dimTick(at: Pt, dir: Pt, size: number, color = INK): SheetPrimitive {
  ...
  return { t: "line", a: { x: at.x - ex, y: at.y - ey }, b: { x: at.x + ex, y: at.y + ey }, w: 0.7, color };
}

/** A north arrow (points primitives) centred at `at`, height `h` pt. */
export function northArrow(at: Pt, h: number): SheetPrimitive[] {
  return [
    { t: "line", a: { x: at.x, y: at.y + h }, b: { x: at.x, y: at.y }, w: 1, color: INK },
    { t: "polygon", pts: [{ x: at.x, y: at.y }, { x: at.x + h * 0.16, y: at.y + h * 0.4 }, { x: at.x, y: at.y + h * 0.28 }, { x: at.x - h * 0.16, y: at.y + h * 0.4 }], fill: INK, stroke: INK, w: 0.3 },
    { t: "text", at: { x: at.x, y: at.y - 3 }, text: "N", size: h * 0.38, color: INK, anchor: "middle", weight: 700 },
  ];
}
