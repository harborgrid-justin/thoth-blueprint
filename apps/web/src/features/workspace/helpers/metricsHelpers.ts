...
export function sumNetworkMeters(
  networks: any[],
  kindFilter: (kind: string) => boolean,
): number {
  return networks
    .filter((n) => kindFilter(n.kind))
    .reduce((s, n) => s + n.lengthMeters, 0);
}

export function formatInfrastructureReadout(
  roadMeters: number,
  ...
  const roadRatio = formatRatio(
    roadMeters / Math.max(1, roadMeters + utilityMeters),
  );
  const roadPercent = formatPercent(
    roadMeters / Math.max(1, roadMeters + utilityMeters),
    0,
  );
...
export function formatAreaSliceReadout(
  area: number,
  share: number,
  unit: AreaUnit,
) {
