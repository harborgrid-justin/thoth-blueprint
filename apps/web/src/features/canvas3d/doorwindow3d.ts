...
export function enterpriseDoor(
  ...
  const woodMat = new THREE.MeshStandardMaterial({
    color:
      door.frameProfile === "vinyl"
        ? 0xe2e8f0
        : door.frameProfile === "metal"
          ? 0x64748b
          : 0x7c2d12,
    roughness: 0.7,
    metalness: 0.1,
  ...
  }
  {
    ...
    hp.position.set(
      hpPos.x - center.x,
      baseElevation + height * exag - (frameWidth / 2) * exag,
      hpPos.y - center.y,
    );
  ...
  // 2. Door Panel (rotated around hinge pLeft)
  {
    ...
    if (door.doorOperation === "swing") {
      ...
      door.doorOperation === "pocket" ||
      door.doorOperation === "slide"
    ) {
    ...
    if (door.hardwareTrim) {
      ...
      if (door.doorOperation === "swing") {
        ...
        kp = add(
          kp,
          scale({ x: Math.cos(angle), y: Math.sin(angle) }, panelW * 0.85),
        );
      } else {
  ...
  // 3. Sill / Threshold blocks
  {
    ...
    m.position.set(
      sillPos.x - center.x,
      baseElevation + (sillThick * exag) / 2,
      sillPos.y - center.y,
    );
...
export function enterpriseWindow(
  ...
  const frameMat = new THREE.MeshStandardMaterial({
    color:
      win.frameProfile === "vinyl"
        ? 0xf1f5f9
        : win.frameProfile === "metal"
          ? 0x475569
          : 0x7c2d12,
    roughness: 0.5,
  });
