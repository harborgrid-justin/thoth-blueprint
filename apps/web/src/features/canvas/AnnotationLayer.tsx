    ...
    <g>
      ...
      {sub != null ? (
        <>
          ...
          <text x={x} y={y + 8} fontSize={7} textAnchor="middle" fill={INK}>
            {sub}
          </text>
        </>
      ) : (
...
export function AnnotationLayer({
  ...
  const keynoteNumber = (id: string) =>
    ann.keynotes?.find((k) => k.id === id)?.number ?? "?";
    ...
    <g className="pointer-events-none">
            ...
            {[a, b].map((end, i) => (
              <g key={i}>
                ...
                <path
                  ...
                  fill={INK}
                  transform={`translate(${end.x + gaze.x * 16} ${end.y + gaze.y * 16}) rotate(${(Math.atan2(gaze.y, gaze.x) * 180) / Math.PI})`}
                />
              </g>
            ))}
          </g>
      ...
      {(ann.revisionClouds ?? []).map((rc) => {
        ...
        return (
          <g key={rc.id}>
            <polyline
              points={`${pts} ${apexes.length ? `${apexes[0].x},${apexes[0].y}` : ""}`}
              fill="none"
              stroke={RED}
              strokeWidth={1.2}
              vectorEffect="non-scaling-stroke"
      ...
      {(ann.keynoteTags ?? []).map((kt) => {
        ...
        return (
          <g key={kt.id}>
            ...
            <path
              d="M0 -10 L9 -5 L9 5 L0 10 L-9 5 L-9 -5 Z"
              fill={HALO}
              stroke={INK}
              strokeWidth={1.1}
