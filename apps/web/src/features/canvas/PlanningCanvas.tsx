...
import { useCanvasViewportController, type Size } from "./hooks/useCanvasViewport";
  ...
  return (
    ...
    >
      <svg width={size.width} height={size.height} className="absolute inset-0 select-none">
        <CanvasPatterns />
        {underlay?.visible && <UnderlayImage underlay={underlay} viewport={viewport} />}
        {showGrid && <Grid viewport={viewport} size={size} step={gridStep} />}
        <FrameworkLayer site={site} viewport={viewport} />
        {clouds.map((c) => c.visible && <CloudDots key={c.id} points={c.cloud.points} viewport={viewport} />)}
        ...
        {ordered.map(({ element }) => {
          const shifted = selectionSet.has(element.id) && (moveDelta.x !== 0 || moveDelta.y !== 0);
          const previewBoundary = vertexPreview?.id === element.id ? vertexPreview.boundary : undefined;
        ...
        {viewFrames.map((f: any) => {
          ...
          return (
            <g key={f.id}>
              <polygon points={pts} fill="none" stroke="#3b82f6" strokeWidth={1.8} strokeDasharray="6 4" />
              <text x={screenPoints[0].x} y={screenPoints[0].y - 6} fill="#3b82f6" fontSize={10} fontWeight="bold">
                {f.name}
              </text>
            </g>
          );
        ...
        {matchLines.map((m: any) => {
          ...
          return (
            <g key={m.id}>
              <line x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke="#ef4444" strokeWidth={2} strokeDasharray="10 5" />
              <text x={(p1.x + p2.x) / 2} y={(p1.y + p2.y) / 2 - 8} textAnchor="middle" fill="#ef4444" fontSize={9} fontWeight="bold">
                {m.label}
              </text>
            </g>
          );
        ...
        {showSurveyLabels && <SurveyEdgeLabels site={site} selection={selection} viewport={viewport} preview={vertexPreview} />}
        <VertexHandles site={site} selection={selection} viewport={viewport} preview={vertexPreview} />
        ...
        {draft.length > 0 && (
          <DraftOverlay draft={draft} cursor={cursor} viewport={viewport} kind={tool.kind ?? tool.label} polyline={tool.mode === "polyline"} />
        )}

        {waterDropPath && waterDropPath.length > 1 && (
          <g className="pointer-events-none">
            <polyline
              points={waterDropPath.map((p) => {
                const s = worldToScreen(p, viewport);
                return `${s.x},${s.y}`;
              }).join(" ")}
              fill="none"
        ...
        {measure.length > 0 && <MeasureOverlay points={measure} cursor={cursor} viewport={viewport} spatial={site.spatial} />}
      </svg>
...
function Grid({ viewport, size, step }: { viewport: Viewport; size: Size; step: number }) {
  ...
  for (let x = startWorldX; x <= endWorldX; x += step) {
    ...
    lines.push(
      <line key={`vx${idx++}`} x1={sx} y1={0} x2={sx} y2={size.height} stroke={`hsl(var(${major ? "--canvas-grid-major" : "--canvas-grid"}))`} strokeWidth={major ? 1 : 0.5} />,
    );
  }
  for (let y = startWorldY; y <= endWorldY; y += step) {
    ...
    lines.push(
      <line key={`hz${idx++}`} x1={0} y1={sy} x2={size.width} y2={sy} stroke={`hsl(var(${major ? "--canvas-grid-major" : "--canvas-grid"}))`} strokeWidth={major ? 1 : 0.5} />,
    );
  }
  return <g>{lines}</g>;
}
