        ...
        <main className="mx-auto max-w-6xl px-5 py-8">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Collaborative site &amp; community plans. Open one to draw parcels, zones, and land uses,
              and watch metrics update live.
            </p>
          </div>
    ...
    <Card className="group relative overflow-hidden transition-colors hover:border-primary/40">
      <Link to={`/project/${project.id}`} className="block">
        ...
        <div className="p-4">
          ...
          <p className="mt-1 line-clamp-2 h-9 text-xs text-muted-foreground">{project.description}</p>
          <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
            <span className="tabular-nums">{formatNumber(project.siteAreaAcres, 1)} ac</span>
            <span>·</span>
            <span className="tabular-nums">{formatNumber(project.lotCount)} lots</span>
      ...
      <div className="absolute right-2 top-2 opacity-0 transition-opacity group-hover:opacity-100">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" className="bg-card/80" aria-label="Project menu">
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-24 text-center">
      ...
      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
        Create your first plan to start laying out parcels, zones, and land uses on a collaborative
        canvas.
      </p>
