...
Business requirements state **why the product exists** â€” the outcomes it must
...
| `BR-005` | The product shall **interoperate with the formats planners already use** â€” import and export GeoJSON, KML, Shapefile, DXF/DWG, GeoPackage, CSV, and PDF exhibits. | Planners will not adopt a tool that cannot read/write their ecosystem; interop is a first-class service. VISION principle 4. | M | `STK-001`,`STK-004`,`STK-007`; `IOP-*`,`BE-IMPORT`,`BE-EXPORT` |
| `BR-006` | The product shall offer **CAD-grade precision** â€” snapping, constraints, coordinate entry, and measurement â€” without CAD-grade friction. | "Approachable power": precision is required for real planning, ease is required for adoption. VISION principle 5. | M | `STK-001`,`STK-004`; `FE-PRECISION`,`FE-MEASURE`,`FE-SELECT` |
