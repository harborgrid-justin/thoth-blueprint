...
export function parsePointCloud(data: PointCloudData, format: PointCloudFormat): PointCloud {
  switch (format) {
    case "xyz":
...
export function serializePointCloud(cloud: PointCloud, format: PointCloudFormat): PointCloudData {
  switch (format) {
    case "xyz":
...
function plyPointFromValues(props: PlyProperty[], values: number[]): CloudPoint {
...
function readPlyScalar(view: DataView, off: number, type: string, le: boolean): [number, number] {
  switch (type) {
    case "char":
    case "int8":
...
export function writePLY(cloud: PointCloud): string {
  ...
  const body = cloud.points.map(
    (p) => `${num(p.x)} ${num(p.y)} ${num(p.z)} ${p.r ?? 255} ${p.g ?? 255} ${p.b ?? 255}`,
  );
  return [...header, ...body].join("\n");
}
...
export function writeDXF(cloud: PointCloud): string {
  ...
  for (const p of cloud.points) {
    ...
    if (p.r != null && p.g != null && p.b != null) {
      out.push("420", String(((p.r & 0xff) << 16) | ((p.g & 0xff) << 8) | (p.b & 0xff)));
    }
  }
...
export function pointCloudElevationRange(cloud: PointCloud): { min: number; max: number } {
  let min = Infinity;
  let max = -Infinity;
  for (const p of cloud.points) {
    if (p.z < min) {min = p.z;}
...
export function pointCloudToSpots(cloud: PointCloud, layerId: string): SpotElevationPoint[] {
...
export function spotsToPointCloud(spots: SpotElevationPoint[]): PointCloud {
  return { points: spots.map((s) => ({ x: s.position.x, y: s.position.y, z: s.z })) };
}
