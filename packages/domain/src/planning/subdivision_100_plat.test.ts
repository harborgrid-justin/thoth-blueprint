...
import { splitPolygonByLine, subdivideSlideLine, subdivideSwingLine, mergeLots } from "./subdivision";

describe("100 Realistic Plat Design & Subdivision Test Scenarios", () => {
  const generateRealisticPolygon = (sides: number, radius: number, cx: number, cy: number): Polygon => {
    const poly: Polygon = [];
    ...
    for (let i = 1; i <= 25; i++) {
      it(`Slide Line Scenario ${i}: Polygon with ${i % 5 + 3} sides, target area ${1000 * i}`, () => {
        ...
        for (let j = 0; j < sides; j++) {
          ...
          poly.push({ x: 500 + radius * Math.cos(angle), y: 500 + radius * Math.sin(angle) });
        }
  ...
  describe("Swing Line Subdivisions (25 Scenarios)", () => {
    for (let i = 26; i <= 50; i++) {
      it(`Swing Line Scenario ${i}: Polygon with ${i % 4 + 4} sides, pivot rotation`, () => {
        ...
        for (let j = 0; j < sides; j++) {
          const angle = (j * 2 * Math.PI) / sides;
          poly.push({ x: 1000 + radius * Math.cos(angle), y: 1000 + radius * Math.sin(angle) });
        }
  ...
  describe("Lot Consolidation (25 Scenarios)", () => {
    for (let i = 76; i <= 100; i++) {
      it(`Lot Merge Scenario ${i}: Merging adjacent realistic subdivisions`, () => {
        ...
        const lot1 = { id: `m1-${i}`, kind: "lot" as const, name: "Lot 1", layerId: "plat", boundary: split![0] };
        const lot2 = { id: `m2-${i}`, kind: "lot" as const, name: "Lot 2", layerId: "plat", boundary: split![1] };
        
        let counter = 0;
        const merged = mergeLots([lot1, lot2], "plat", () => `merged-${i}-${++counter}`);
