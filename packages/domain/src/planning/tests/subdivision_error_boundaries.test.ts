...
import { subdivideSlideLine, subdivideSwingLine, mergeLots } from "../subdivision";
...
describe("Plat Design Boundary & Error Validation", () => {
  ...
  describe("Slide Line Boundary Restrictions", () => {
    it("should throw error when target area exceeds total parcel area", () => {
      ...
      }).toThrowError(/target area \(15000\.00\) exceeds total parcel area \(10000\.00\)/);
    });
  ...
  describe("Swing Line Boundary Restrictions", () => {
    ...
    it("should throw error for invalid polygon boundaries (less than 3 points)", () => {
      const lineBoundary: Polygon = [{ x: 0, y: 0 }, { x: 100, y: 0 }];
      expect(() => {
  ...
  describe("Lot Merging Errors", () => {
    it("should throw error when no lots are provided", () => {
      expect(() => mergeLots([], "test", () => "merged")).toThrowError("No lots provided for merge");
    });
    
    it("should throw error when lots do not share an adjacent boundary", () => {
      const lot1 = { id: "1", kind: "lot" as const, name: "l1", layerId: "test", boundary: validSquare };
      ...
      const lot2 = { id: "2", kind: "lot" as const, name: "l2", layerId: "test", boundary: nonAdjacentSquare };
      
      expect(() => mergeLots([lot1, lot2], "test", () => "merged"))
